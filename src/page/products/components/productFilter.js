...
const ProductFilter = ({setShow}) => {
    ...
    function handleBestSeller() {
        const newBestSellerState = !state.bestSellerOnly;
        dispatch({
            type: 'BEST_SELLER_ONLY',
            payload: { bestSellerOnly: newBestSellerState },
        });
        console.log("Best Seller Only:", newBestSellerState);
    }
        
    function handleInStock() {
        const newInStockState = !state.inStockOnly;
        dispatch({
            type: 'IN_STOCK_ONLY',
            payload: { inStockOnly: newInStockState },
        });
        console.log("in Stock Only:", newInStockState);
    }
            ...
            <div id="drawer-disable-body-scrolling" className={`fixed z-40 h-screen p-5 overflow-y-auto bg-white w-72 dark:bg-gray-800 transition-transhtmlForm left-0 top-0 transhtmlForm-none`} tabIndex="-1" aria-labelledby="drawer-disable-body-scrolling-label" aria-modal="true" role="dialog">
                ...
                <div className="py-4 overflow-y-auto">
                    ...
                    <li className="mt-1 mb-5">
                        ...
                        <div className="flex items-center my-1">
                            <input  onChange={handleBestSeller}  checked={state.bestSellerOnly ?? false}  id="best-seller" type="checkbox" value="" className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                            <label htmlFor="best-seller" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Best Seller Only</label>
                        </div>
                        <div className="flex items-center my-1">
                            <input  onChange={handleInStock}   checked={state.inStockOnly ?? false}   id="only-instock" type="checkbox" value="" className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
