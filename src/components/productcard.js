    ...
    return ( 
        ...
        <div className="p-5">
            ...
            <p className="flex justify-between items-center">
                ...
                :(<button onClick={() => addToCart(product)} className={`inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 ${product.in_stock ? '' : "cursor-not-allowed"}`} disabled={product.in_stock ? "" : 'disable'}>Add To Cart <i className="ml-1 bi bi-plus-lg"></i></button>)}
            </p>
        </div>
    </div>
     );
